...
BASE_DIR = r"c:\Users\gg261\Videos\Capstone Project Details\BUSBRA\BUSBRA"
...
def main():
    ...
    best_checkpoint_file = f'model_epoch_{(best_epoch // 10) * 10}.pth'
    if os.path.exists(os.path.join(CHECKPOINT_DIR, best_checkpoint_file)):
        print(f"\nNOTE: Best epoch was {best_epoch}")
        print(f"Closest saved checkpoint: {best_checkpoint_file}")
        print(f"Use this checkpoint for inference")
    else:
        print(f"\nNOTE: Exact best epoch checkpoint not saved (saved every 10 epochs)")
    ...
    print("You can analyze this file to find the exact best epoch")
